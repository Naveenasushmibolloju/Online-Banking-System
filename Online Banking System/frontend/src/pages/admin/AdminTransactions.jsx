...
import { FiSearch, FiFilter, FiRefreshCw, FiAlertCircle, FiArrowRight } from 'react-icons/fi'
  ...
  return (
    <div className="space-y-6 max-w-7xl animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-1">All Transactions</h1>
          <p className="text-gray-600">Monitor system transactions</p>
        </div>
        <button onClick={loadTransactions} className="btn-secondary flex items-center gap-2 self-start">
          <FiRefreshCw size={16} />
          Refresh
        </button>
      </div>
      ...
      <div className="card p-6">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <select value={filters.type} onChange={(e) => setFilters(prev => ({ ...prev, type: e.target.value }))} className="input">
            ...
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-gray-50/50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Receiver</th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {transactions.map(txn => (
                  <tr key={txn._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{txn.senderId?.firstName} {txn.senderId?.lastName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{txn.receiverId?.firstName} {txn.receiverId?.lastName}</td>
